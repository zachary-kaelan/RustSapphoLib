use crate::consts::BNUM_GROUP_SIZE;

pub const PERSONALITY_VALUE_NAMES: [&str; BNUM_GROUP_SIZE] = ["bad_good", "faithless_honest", "timid_dominant", "ascetic_hedonistic"];
pub const PERCEPTION_VALUE_NAMES: [&str; BNUM_GROUP_SIZE] = ["hate_love", "mistrust_faith", "disregard_respect", "disinterest_indulgence"];
pub const ACCORDANCE_VALUE_NAMES: [&str; BNUM_GROUP_SIZE] = ["misanthropy_altruism", "paranoia_gullibility", "helplessness_control", "austerity_decadence"];
pub const VERB_VALUE_NAMES: [&str; BNUM_GROUP_SIZE] = ["selfish_altruistic", "deceptive_sincere", "submissive_dominant", "cold_emotional"];
